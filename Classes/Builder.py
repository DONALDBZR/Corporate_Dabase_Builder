...
class Builder:
    ...
    def firstRun(self) -> None:
        ...
        data: tuple[int, str, str, str] = self.getDatabaseHandler().get_data(
            ...
            filter_condition=f"CONCAT(YEAR({str(self.getDate())}), '-', start_date) < {str(self.getDate())} AND CONCAT(YEAR({str(self.getDate())}), '-', end_date) > {str(self.getDate())}",
            column_names=f"YEAR({str(self.getDate())}) AS year, quarter, CONCAT(YEAR({str(self.getDate())}), '-', start_date) AS start_date, CONCAT(YEAR({str(self.getDate())}), '-', end_date) AS end_date"
        )[0]
        quarter = {
            "year": int(data[0]),
            "quarter": str(data[1]),
