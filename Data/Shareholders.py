"""
The Data Transfer Object for the Shareholder.
"""


from dataclasses import dataclass
from mysql.connector.types import RowType
from typing import Union, Dict


@dataclass
class Shareholder:
    """
    The Data Transfer Object for the Shareholder.
    """
    identifier: int
    CompanyDetail: int
    name: str
    amount_shares: int
    type_shares: str
    currency: str

    def __init__(self, dataset: Union[RowType, Dict[str, Union[int, str]]]) -> None:
        """
        Initializing the data class object.

        Parameters:
            dataset: {identifier: int, CompanyDetail: int, name: string, amount_shares: int, type_shares: string, currency: string}: The result set from the relational database server.

        Returns:
            void
        """
        self.identifier = int(dataset["identifier"]) # type: ignore
        self.CompanyDetail = int(dataset["CompanyDetail"]) # type: ignore
        self.name = str(dataset["name"]) # type: ignore
        self.amount_shares = int(dataset["amount_shares"]) # type: ignore
        self.type_shares = str(dataset["type_shares"]) # type: ignore
        self.currency = str(dataset["currency"]) # type: ignore