...
class Builder:
    ...
    def collectCorporateMetadata(self) -> None:
        ...
        if len(successful_logs) == 1 and successful_logs[0].status == 204:
            date_to: str = datetime.strftime(datetime.strptime(quarter.start_date, "%m/%d/%Y") + timedelta(weeks=52), "%m/%d/%Y")
            request = {
    ...
    def handleRequestCollectCorporateMetadata(self, logs: List[FinCorpLogs]) -> Dict[str, str]:
        ...
        date_end = datetime.strftime(datetime.strptime(date_start, "%m/%d/%Y") + timedelta(weeks=8), "%m/%d/%Y")
        ...
        if date_end_unixtime > current_time:
            ...
            date_start = datetime.strftime(datetime.strptime(date_end, "%m/%d/%Y") - timedelta(weeks=8), "%m/%d/%Y")
        return {
